...
export default function Navbar() {
  ...
  // --- STYLE CONTROLS ---
  // If Home: Transparent. 
  // If App (White background pages): White with a strong border/shadow.
  const navBg = isHome 
    ? 'bg-transparent border-transparent' 
    : 'bg-white border-b border-slate-200 shadow-sm'
  
  const textColor = isHome ? 'text-white' : 'text-slate-900'
          ...
          <div className="hidden md:flex items-center space-x-8">
            {user ? (
              <>
                ...
                <div className="flex items-center gap-4 pl-6 border-l border-slate-200">
                  ...
                  <button 
                    ...
                    className="bg-primary text-white px-4 py-2 rounded-lg font-bold text-sm hover:bg-slate-800 transition-all"
                  >
                    Log Out
                  </button>
                </div>
              ...
              <div className="flex items-center space-x-6">
                <Link href="/login" className={`${textColor} font-bold hover:text-secondary transition-colors`}>
                  Sign In
                </Link>
                <Link 
                  href="/login?mode=signup" 
                  className="bg-primary text-white px-6 py-2.5 rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg shadow-primary/10"
                >
                  Sign Up
                </Link>
              </div>
